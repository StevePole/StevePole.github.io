...
function ApiService($window, $http, $q, $routeParams, AuthService, ApiService2) {
    var self = this;
    this.local = $window.localStorage, this.apiListKey = "swagger-existing", this.list = [ "json/cds_swagger.json", "json/auth.json", "json/swift.json" ], 
...
function Swagger(Utility) {
    ...
    this.findOperationByPathAndMethod = function(apis, path, method) {
        ...
        _.each(json.apis, function(api) {
            if (!api.parentModel) {
                if (api.child) {
                    if (json.apis["/" + api.parent]) {
                        var parentApi = json.apis["/" + api.parent];
                        api.parentModel = swagger.prepInferParentModel(parentApi), Utility.log(parentApi.path + " " + api.parentModel);
                    } else if (json.apis["/" + api.parent + "/{id}"]) {
                        var parentApi = json.apis["/" + api.parent + "/{id}"];
                        api.parentModel = swagger.prepInferParentModel(parentApi);
                    } else Utility.log("Counld not find /" + api.parent + " or /" + api.parent + "/{id}");
                    return void (api.parentModel || Utility.log("Could not find a parent for " + api.path));
                }
        ...
        return property.isDictionary ? this.dictionaryExampleJson(models, property, depth, requestType, path) : property.isArray ? this.arrayExampleJson(models, property, depth, requestType, path) : this.singleExampleJson(models, property, depth, requestType, path);
    }, this.dictionaryExampleJson = function(models, property, depth, requestType, path) {
        if (property.example) return {
        ...
        if (property.items && property.items.$ref) {
            var nestedModel = models[property.items.$ref];
            return nestedModel ? this.containsRecursion(path, property.items.$ref) ? "..." : (path.push(property.items.$ref), 
            {
                example: this.modelExampleJson(models, nestedModel, depth + 1, !1, requestType, !1, path)
            }) : {
                example: "Model not found"
        ...
        return "enum" === property.displayType && property["enum"] && property["enum"].length ? {
        ...
        if (property.example) return [ this.formatNumber(property.items.type, property.example) ];
        if ("object" === property.displayType) {
            var nestedModel = models[property.displayModel];
            return nestedModel ? this.containsRecursion(path, property.displayModel) ? [ "..." ] : (path.push(property.displayModel), 
        ...
        return "enum" === property.displayType && property["enum"] && property["enum"].length ? [ property["enum"][0] ] : this.displayDefaults.hasOwnProperty(property.displayType) ? property.example || "boolean" === property.displayType && void 0 !== property.example ? [ property.example ] : [ this.displayDefaults[property.displayType] ] : [ "Type not found" ];
...
function(angular) {
    ...
    function SwaggerValidate() {
        ...
        this.validateAuth = function(errors, authorizations) {
            ...
            if (!model.properties, "array" == typeof model.properties) throw new Error("Model properties should not be an array at this point.");
            var idSet = !1;
            _.each(model.properties, function(property, propertyName) {
