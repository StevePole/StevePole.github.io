...
function ApiDesigner() {
    this.blankJson = function() {
        ...
        switch (pathType) {
          case Swagger.pathTypes.parent:
            api.description = "Interact with the list of resources.";
            break;

          case Swagger.pathTypes.parentId:
            api.description = "Interact with a single resource.";
            break;

          case Swagger.pathTypes.parentIdChild:
            api.description = "Interact with the child resources related to a specific parent.";
            break;

          case Swagger.pathTypes.parentChild:
            api.description = "Interact with the child resources related to a specific parent type.";
        }
        return api;
        ...
        switch (method) {
          ...
          case "HEAD":
            action = "Retrieve headers and meta data about ";
            break;
        ...
        switch (pathType) {
          case Swagger.pathTypes.parent:
            operation.isArray = "POST" === method ? !1 : !0, thing = "the collection", how = "";
            break;

          case Swagger.pathTypes.parentId:
            thing = "a single resource", how = " by id";
            break;

          case Swagger.pathTypes.parentIdChild:
            operation.isArray = "POST" === method ? !1 : !0, thing = "the child collection", 
            how = " related to a parent resource";
            break;

          case Swagger.pathTypes.parentChild:
            operation.isArray = "POST" === method ? !1 : !0, thing = "the child collection", 
            how = "for the parent resource type";
        }
        return operation.summary = action + thing + how + ".", operation;
...
function Swagger(Utility) {
    ...
    this.findOperationByPathAndMethod = function(apis, path, method) {
        ...
        _.each(model.properties, function(property) {
            ...
            property.isId && (model.idProperty = property.name), property.idModel && (property.isRelatedModelId = !0);
        ...
        delete typeObject.isRelatedModelId, (!typeObject["enum"] || typeObject["enum"] instanceof Array && !typeObject["enum"].length) && delete typeObject["enum"];
        ...
        var property = this.getModelIdPropertyByModelName(models, modelName);
        return property.name;
    }, this.getModelIdPropertyByModelName = function(models, modelName) {
        var model = this.getModelByName(models, modelName);
        if (model) return this.findModelIdProperty(model);
        ...
        return property.isDictionary ? this.dictionaryExampleJson(models, property, depth, requestType, pathClone) : property.isArray ? this.arrayExampleJson(models, property, depth, requestType, pathClone) : property.isRelatedModelId ? this.relatedModelIdExampleJson(models, property, depth, requestType, pathClone) : this.singleExampleJson(models, property, depth, requestType, pathClone);
    ...
    }, this.relatedModelIdExampleJson = function(models, property, depth, requestType, path) {
        var idProperty = this.getModelIdPropertyByModelName(models, property.idModel);
        return console.log("id model: " + property.idModel), console.log("idProperty: " + idProperty.name), 
        console.log(idProperty), this.singleExampleJson(models, idProperty, depth, requestType, path);
...
function(angular) {
    function SwaggerMerge() {
        function merge(baseJson, addJson) {
            return mergeModels(baseJson.models, addJson.models), mergeApis(baseJson.apis, addJson.apis), 
            baseJson;
        }
        function mergeModels(baseModels, addModels) {
            angular.forEach(addModels, function(addModel, modelName) {
                baseModels[modelName] ? mergeModelProperties(baseModels[modelName], addModel) : baseModels[modelName] = angular.copy(addModel);
            });
        }
        function mergeModelProperties() {}
        function mergeApis(baseApis, addApis) {
            var basePaths = {};
            angular.forEach(baseApis, function(baseApi, index) {
                basePaths[baseApi.path] = index;
            }), angular.forEach(addApis, function(addApi) {
                var apiIndex = basePaths[addApi.path];
                apiIndex ? (mergeApiOperations(baseApis[apiIndex], addApi), mergeApiParameters(baseApis[apiIndex], addApi)) : baseApis.push(angular.copy(addApi));
            });
        }
        function mergeApiOperations() {}
        function mergeApiParameters() {}
        var self = this;
        self.merge = merge;
    }
    angular.module("apis-application.services").service("SwaggerMerge", SwaggerMerge), 
    SwaggerMerge.$inject = [ "Utility" ];
...
function(angular) {
    ...
    function Utility() {
        ...
        }, self.arrayUp = function(array, fromIndex) {
            self.arrayMove(array, fromIndex, fromIndex - 1);
        }, self.arrayDown = function(array, fromIndex) {
            self.arrayMove(array, fromIndex, fromIndex + 1);
        }, self.arrayTop = function(array, fromIndex) {
            self.arrayMove(array, fromIndex, 0);
        }, self.arrayBottom = function(array, fromIndex) {
            self.arrayMove(array, fromIndex, array.length - 1);
        }, self.remove = function(collection, idx) {
